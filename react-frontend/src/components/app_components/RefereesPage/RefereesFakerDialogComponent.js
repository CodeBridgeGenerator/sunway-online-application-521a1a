...
import refereesFakerFactory from "./refereesFakerFactory";
...
const RefereesFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("applicantDetails")
    ...
    const onRun = async () => {
        let fakeData = refereesFakerFactory(props.user,fakerCount,applicantIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("referees").create(elem));
...
export default connect(mapState, mapDispatch)(RefereesFakerDialogComponent);
