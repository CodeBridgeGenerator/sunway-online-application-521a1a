...
import applicantDetailsFakerFactory from "./applicantDetailsFakerFactory";
...
const ApplicantDetailsFakerDialogComponent = (props) => {
    ...
    useEffect(() => {
        ...
        client.service("studentUser")
                        .find({ query: { $select: ["_id","passport"] } })
                        .then((data) => {
                        setstudentUserIds(data.data);
                        });
    ...
    const onRun = async () => {
        let fakeData = applicantDetailsFakerFactory(props.user,fakerCount,studentUserIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("applicantDetails").create(elem));
...
export default connect(mapState, mapDispatch)(ApplicantDetailsFakerDialogComponent);
