...
const ContactDetailsFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = contactDetailsFakerFactory(props.user,fakerCount,applicantIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("contactDetails").create(elem));
...
export default connect(mapState, mapDispatch)(ContactDetailsFakerDialogComponent);
