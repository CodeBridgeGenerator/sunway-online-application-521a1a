...
const ProgrammesCreateDialogComponent = (props) => {
    ...
    const onSave = async () => {
        if(!validate()) return;
        let _data = {
            campus: _entity?.campus,location: _entity?.location,programmeLevel: _entity?.programmeLevel,programme: _entity?.programme,intake: _entity?.intake,
        ...
        const result = await client.service("programmes").create(_data);
        props.onHide();
        props.alert({ type: "success", title: "Create info", message: "Info Programmes created successfully" });
        props.onCreateResult(result);
        } catch (error) {
            console.log("error", error);
            setError(getSchemaValidationErrorsStrings(error) || "Failed to create");
            props.alert({ type: "error", title: "Create", message: "Failed to create in Programmes" });
        }
        setLoading(false);
    };
    ...
    return (
        <Dialog header="Create Programmes" visible={props.show} closable={false} onHide={props.onHide} modal style={{ width: "40vw" }} className="min-w-max scalein animation-ease-in-out animation-duration-1000" footer={renderFooter()} resizable={false}>
            <div className="grid p-fluid overflow-y-auto"
            style={{ maxWidth: "55vw" }} role="programmes-create-dialog-component">
...
<div className="col-12 md:col-6 field">
            <span className="align-items-center">
                ...
                <Calendar id="intake"  value={_entity?.intake ? new Date(_entity?.intake) : null} dateFormat="dd/mm/yy" onChange={ (e) => setValByKey("intake", new Date(e.target.value))} showIcon showButtonBar  />
            </span>
            <small className="p-error">
