...
import programmesFakerFactory from "./programmesFakerFactory";
...
const ProgrammesFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = programmesFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("programmes").create(elem));
...
export default connect(mapState, mapDispatch)(ProgrammesFakerDialogComponent);
