...
import studentUsersFakerFactory from "./studentUsersFakerFactory";
...
const StudentUsersFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = studentUsersFakerFactory(props.user,fakerCount,userIds);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("studentUsers").create(elem));
...
export default connect(mapState, mapDispatch)(StudentUsersFakerDialogComponent);
